...
var eurecaServer = new Eureca.Server({allow:['recibirPedido']});

eurecaServer.attach(server);
var reina;

eurecaServer.exports.holaReina = function (){
  reina = this.clientProxy;
  console.log("La hormiga Reina se conecto");
}
eurecaServer.exports.peticionComida = function (peso,tipo,alma){
  ...
  for(var i=0;i<hormis.length;i++){
       console.log("Se envio una hormiga a buscar comida en: " + alma);
...
console.log(inventarios);
   ...
   if(pedidos[id].listo()){
     ...
     //Si la hormiga no esta lista
       console.log("LLego una hormiga que no pudo satisfacer la demanda.");
...
var generador1 = new Eureca.Client({ uri: 'http://192.168.0.5:8081' });
var generador2 = new Eureca.Client({ uri: 'http://192.168.0.5:8082' });
var generador3 = new Eureca.Client({ uri: 'http://192.168.0.5:8083' });
...
function crearPedido(papas,cannabis,tomate,callback){
 ...
 if(papas>0){
  ...
  if(cannabis>0){
   ...
   pedidos.push(pedido);
   nroPedido++;
   console.log("Se creo este pedido");
   console.log(pedido);
